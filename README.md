...
**The overlay is gitignored, not committed.** `afk:init`/`afk:update` treat the installed runtime overlay (`.sandcastle/`, `bin/forge`, the layer's `skills/agent-ready-issue/` + `scripts/*`) like `node_modules`: materialized on disk but kept **out of your git history** via a managed `.gitignore` block, so a layer refresh never shows up as a tracked diff. The classification lives in `.sandcastle/.managed.json` (`ignore` vs `commit`). The exception is **CI-class** files — the auto-unblock workflow and `scripts/auto-unblock/*` — which stay committed because GitHub Actions only run committed files. Upgrading an existing consumer that had the overlay committed: the next `afk:update` untracks those files (`git rm --cached`, left on disk) — commit that one-time cleanup and you're clean thereafter.
